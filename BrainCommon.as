...
CBlob@ getNewTarget(CBrain@ this, CBlob @blob, const bool seeThroughWalls = false, const bool seeBehindBack = false)
{
	...
	for (uint i = 0; i < players.length; i++)
	{
		...
		if (potential !is blob && blob.getPlayer().getTeamNum() != potential.getPlayer().getTeamNum() && !potential.hasTag("dead"))
			/*&& (isBot || seeBehindBack || Maths::Abs(pos.x - pos2.x)<30.f)
			&& (isBot || seeThroughWalls || isVisible(blob, potential))
			&& (!potential.hasTag("dead") && !potential.hasTag("migrant")))*/
		{
			Vec2f point = potential.getPosition();
			float curr_dist = (point - mypos).Length();
			if (curr_dist<min_dist)
			{
				blob.set_Vec2f("last pathing pos", potential.getPosition());
				@final_target = potential;
				min_dist = curr_dist;
			}
...
bool isLineOfSight(Vec2f targetPos,float angle,float distance,Vec2f &out otherP)
{
	Vec2f col;
	Vec2f otherPoint;
	otherPoint = Vec2f(	Maths::Cos(angle)*distance/2+targetPos.x,
						-Maths::Sin(angle)*distance/2+targetPos.y);
	return !getMap().rayCastSolid(targetPos, otherPoint, otherPoint);
}

bool isLineOfSight(CBlob@blob,float angle,float distance,Vec2f &out otherP)
{
	Vec2f col;
	Vec2f otherPoint;
	otherPoint = Vec2f(	Maths::Cos(angle)*distance/2+blob.getPosition().x,
						-Maths::Sin(angle)*distance/2+blob.getPosition().y);
	return !getMap().rayCastSolid(blob.getPosition(), otherPoint, col);
}
...
bool JustGo(CBlob@ blob, CBlob@ target)
{
	...
	blob.setKeyPressed((mypos.x<point.x ? key_right : key_left),true);
...
bool JustGo2(CBlob@ blob, CBlob@ target)
{
	Vec2f mypos = blob.getPosition();
	Vec2f point = target.getPosition();
...
void DefaultChaseBlobVale(CBlob@ blob, CBlob @target)
{
	...
	if ((blob.get_Vec2f("last pathing pos") - targetPos).getLength() > 50.0f)
	{
		Repath(brain);
...
void SearchTarget(CBrain@ this, const bool seeThroughWalls = false, const bool seeBehindBack = true)
{
	...
	if ((target is null)or(XORRandom(50)<=20))
	{
