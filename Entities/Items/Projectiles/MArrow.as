...
string Vec2fArray2String(array<Vec2f> arr)
{
	string s = "";
	int i=0;
	for (;i<arr.length();i++){
...
void saveStats()
{
	...
	string cost_config_file = "arrowStats.cfg";
	cfg.loadFile("../Cache/"+cost_config_file);
	string suffix = velocities[0]+"_"+positions[0];
	cfg.add_string("Velocities"+suffix,Vec2fArray2String(velocities));	
	cfg.add_string("Positions"+suffix,Vec2fArray2String(positions));	
...
void onTick(CBlob@ this)
{
	...
	if (shape.vellen>0.001f){
		...
		velocities.insertLast(this.getVelocity());
